...
class KeycloakClientTestCase(ModuleTestCase):
    ...
    testClients = [{"auth_keycloak_url": "http://localhost:18081/auth",
                    "auth_username": "admin",
                    "auth_password": "admin",
                    "realm": "master",
                    "state": "present",
                   ...
                   {"auth_keycloak_url": "http://localhost:18081/auth",
                    ...
                    "roles": [{"name": "test1",
                              ...
                              {"name": "test2",
                               ...
                               "composite": True,
                               "composites": []}],
                    "protocolMappers": [{"name": "test1Mapper",
                                         "protocol": "openid-connect",
                   ...
                   {"auth_keycloak_url": "http://localhost:18081/auth",
                    ...
                    "publicClient": False,
                    "protocolMappers": [{"name": "thismapperstays",
                                         "protocol": "openid-connect",
                                         "protocolMapper": "oidc-usermodel-attribute-mapper",
                                         "consentRequired": False,
                                         "config": {"multivalued": 'false',
                   ...
                   {"auth_keycloak_url": "http://localhost:18081/auth",
                    ...
                    "enabled": True,
                    "clientAuthenticatorType": "client-secret",
                    "redirectUris": ["http://test.com:8080/secure",
    ...
    def setUp(self):
        ...
        for client in self.testClients:
            if client["clientId"] == "basetest":
                ...
                client['roles'][1]['composites'].append(
                    {'id': self.testClients[0]['clientId'], "name": self.testClientRoles[0]['name']})
    ...
    def test_create_client(self):
        ...
        self.assertEqual(
            OrderdRoles[0]['name'],
            toCreate["roles"][0]['name'],
            "roles : " +
            OrderdRoles[0]['name'])
        self.assertEqual(
            OrderdRoles[1]['name'],
            toCreate["roles"][1]['name'],
            "roles : " +
            OrderdRoles[1]['name'])
        for newComposite in toCreate["roles"][1]['composites']:
            compositeFound = False
            for createdComposite in OrderdRoles[1]['composites']:
                if "id" in newComposite and newComposite["id"] == createdComposite[
                        'id'] and newComposite["name"] == createdComposite['name']:
                    compositeFound = True
    ...
    def test_modify_client(self):
        ...
        toModifyClient["protocolMappers"] = [
            {
                "name": "test1Mapper",
                "protocol": "openid-connect",
                "protocolMapper": "oidc-usermodel-attribute-mapper",
        ...
        self.assertEqual(
            results.exception.args[0]['end_state']['name'],
            toModifyClient["name"],
            "name: " +
            results.exception.args[0]['end_state']['name'])
        self.assertEqual(
            results.exception.args[0]['end_state']['description'],
            toModifyClient["description"],
            'description: ' +
            results.exception.args[0]['end_state']['description'])
        self.assertEqual(
            results.exception.args[0]['end_state']['redirectUris'].sort(),
            toModifyClient["redirectUris"].sort(),
            "redirectUris: " + str(
                results.exception.args[0]['end_state']['redirectUris'].sort()))
        for toChangeMapper in toModifyClient["protocolMappers"]:
            ...
            if mapperFound:
                self.assertEqual(
                    mapper["config"]["claim.name"],
                    toChangeMapper["config"]["claim.name"],
                    "claim.name: " +
                    toChangeMapper["config"]["claim.name"] +
                    ": " +
                    mapper["config"]["claim.name"])
                self.assertEqual(
                    mapper["config"]["user.attribute"],
                    toChangeMapper["config"]["user.attribute"],
                    "user.attribute: " +
                    toChangeMapper["config"]["user.attribute"] +
                    ": " +
                    mapper["config"]["user.attribute"])
        ...
        self.assertEqual(
            OrderdRoles[0]['name'],
            toModifyClient["roles"][0]['name'],
            "roles : " + OrderdRoles[0]['name'])
        self.assertEqual(
            OrderdRoles[1]['name'],
            toModifyClient["roles"][1]['name'],
    ...
    def test_add_client_composite_roles(self):
        ...
        self.assertEqual(
            OrderdRoles[0]['name'],
            newClientRoles[0]['name'],
            "roles : " +
            OrderdRoles[0]['name'])
        self.assertEqual(
            OrderdRoles[1]['name'],
            newClientRoles[1]['name'],
            "roles : " +
            OrderdRoles[1]['name'])
        OrderedComposites = sorted(
            OrderdRoles[1]['composites'],
            key=lambda k: ['name]'])
    ...
    def test_remove_mapper_from_client(self):
        ...
        self.assertFalse(
            mapperFound,
            "Mapper " +
            toRemoveMapperFromClient["protocolMappers"][1]["name"] +
    ...
    def test_remove_role_from_client(self):
        ...
        self.assertFalse(
            roleFound,
            "Role " +
            toRemoveRoleFromClient["roles"][1]["name"] +
            " has not been deleted")
    ...
    def test_create_client_with_non_existing_client_composite_role(self):
        ...
        self.assertRegexpMatches(
            results.exception.args[0]['msg'],
            'client ' +
            toErrorClient["roles"][1]["composites"][1]["id"] +
            ' does not exist',
            'error not generated')
