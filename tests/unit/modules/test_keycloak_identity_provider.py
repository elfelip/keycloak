...
class KeycloakIdentityProviderTestCase(ModuleTestCase):
    ...
    testIDPs = [{"auth_username": "admin",
                 ...
                 "config": {"openIdConfigurationUrl": "http://localhost:18081/auth/realms/master/.well-known/openid-configuration",
                            "clientId": "test",
                ...
                {"auth_username": "admin",
                 ...
                 "config": {"openIdConfigurationUrl": "http://localhost:18081/auth/realms/master/.well-known/openid-configuration",
                            ...
                            "defaultScope": "openid email profile",
                            "disableUserInfo": "false"},
                 "mappers": [{"name": "thismapperissupposedtostay",
                              "identityProviderMapper": "oidc-user-attribute-idp-mapper",
                              "config": {"claim": "newTest",
                                         "user.attribute": "lastname"},
                              "state": "present"},
    ...
    def test_create_idp(self):
        ...
        self.assertEquals(
            ...
            results.exception.args[0]['idp']['alias'])
        self.assertEquals(
            results.exception.args[0]['idp']['config']['clientId'],
            toCreate["config"]["clientId"],
            "ClientId: " +
            results.exception.args[0]['idp']['config']['clientId'])
        self.assertEquals(
            results.exception.args[0]['idp']['config']['guiOrder'],
            toCreate["config"]["guiOrder"],
            "GuiOrder: " +
            results.exception.args[0]['idp']['config']['guiOrder'] +
            ": " +
            toCreate["config"]["guiOrder"])
        self.assertTrue(
            'clientAuthMethod' in results.exception.args[0]['idp']['config'],
            "clientAuthMethod not in IdP config")
        self.assertEquals(
            ...
            for mapper in results.exception.args[0]['mappers']:
                if mapper["name"] == mapperToCreate["name"]:
                    ...
                    self.assertEquals(
                        ...
                        mapperToCreate["identityProviderMapper"],
                        "identityProviderMapper: " +
                        mapper["identityProviderMapper"] +
                        "not equal " +
                        mapperToCreate["identityProviderMapper"])
                    self.assertDictEqual(mapper["config"], mapperToCreate["config"], "config: " + str(
                        mapper["config"]) + "not equal " + str(mapperToCreate["config"]))
            self.assertTrue(
                mapperFound,
                "mapper " +
                mapperToCreate["name"] +
    ...
    def test_idp_not_changed_with_display_name_removed(self):
        ...
        self.assertFalse(results.exception.args[0]['changed'])
        self.assertEquals(
            results.exception.args[0]['idp']['displayName'],
            self.testIDPs[1]['displayName'],
            'displayName changed: ' +
            results.exception.args[0]['idp']['displayName'])
