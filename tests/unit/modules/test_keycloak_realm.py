...
class KeycloakRealmTestCase(ModuleTestCase):
    ...
    def test_create_realm(self):
        ...
        self.assertTrue(
            ...
            "adminEventsEnabled: " + str(
                results.exception.args[0]["eventsConfig"]["adminEventsEnabled"]))
        self.assertTrue(
            results.exception.args[0]["eventsConfig"]["adminEventsDetailsEnabled"],
            "adminEventsDetailsEnabled: " + str(
                results.exception.args[0]["eventsConfig"]["adminEventsDetailsEnabled"]))
        self.assertTrue(
            isDictEquals(
                toCreate['eventsConfig'],
    ...
    def test_do_not_change_realm(self):
        ...
        with self.assertRaises(AnsibleExitJson) as results:
            self.module.main()
        self.assertFalse(
            results.exception.args[0]['changed'],
            "Realm is not supposed to be changed")
        self.assertTrue(
            isDictEquals(
                self.toDoNotChangeRealm,
                results.exception.args[0]['realm'],
                self.realmExcudes),
