...
class KeycloakRoleTestCase(ModuleTestCase):
    ...
    testRoles = [{"auth_username": "admin",
                  ...
                  "composite": True,
                  "composites": [{"clientId": "master-realm",
                                  "name": "manage-clients"},
                 ...
                 {"auth_username": "admin",
                  ...
                  "composite": True,
                  "composites": [{"clientId": "TestUPPER_underscore",
                                  "name": "admin"}],
                  "state": "absent",
                  "force": False},
                 {"auth_username": "admin",
    ...
    def test_role_modify_force(self):
        ...
        self.assertTrue(
            isDictEquals(
                ...
                results.exception.args[0]['role'],
                self.roleExcudes),
            'Realm role not modified force does not comply to specifications.')
        self.assertTrue(
            isDictEquals(
                toDoNotChange["composites"],
                results.exception.args[0]['composites'],
                self.roleExcudes),
            'Realm role composites not modified force does not comply to specifications.')
    ...
    def test_create_role_composites_with_client_role_clientid_contain_uppercase_and_underscore(
            self):
    ...
    def test_create_role_composites_with_non_existing_client(self):
        ...
        self.assertRegexpMatches(
            results.exception.args[0]['msg'],
            'client ' +
            toCreate["composites"][0]["clientId"] +
            ' not found',
            'error not generated: ' +
            results.exception.args[0]['msg'])

    def test_modify_role_composites_existing_client_but_non_existing_role(
            self):
