...
def mocked_requests_post(*args, **kwargs):
    ...
    if kwargs["url"] == "{}/realms/{}/protocol/openid-connect/token".format(keycloak_url, keycloak_auth_realm):
        access_token = jwt.encode(decoded_access_token, jwt_secret, algorithm=jwt_algo, headers={'kid': 'NWBeViRdZb3-n0pBGu5YMJnaV1UMRMLjcvMOPJA2Gko', 'alg': jwt_algo, 'typ': 'jwt'})
        refresh_token = jwt.encode(decoded_refresh_token, jwt_secret, algorithm=jwt_algo, headers={'kid': 'db1c7a43-58fb-421b-828c-98a921eae51d', 'alg': jwt_algo, 'typ': 'jwt'})
        content = {}
...
def mocked_open_url(*args, **kwargs):
    ...
    if args[0] == "{}/realms/{}/protocol/openid-connect/token".format(keycloak_url, keycloak_auth_realm) and kwargs["method"] == 'POST':
        access_token = jwt.encode(decoded_access_token, jwt_secret, algorithm=jwt_algo, headers={'kid': 'NWBeViRdZb3-n0pBGu5YMJnaV1UMRMLjcvMOPJA2Gko', 'alg': jwt_algo, 'typ': 'jwt'})
        refresh_token = jwt.encode(decoded_refresh_token, jwt_secret, algorithm=jwt_algo, headers={'kid': 'db1c7a43-58fb-421b-828c-98a921eae51d', 'alg': jwt_algo, 'typ': 'jwt'})
        content = {}
